...
def path2pools(
    path: Path, wavlm: nn.Module, match_weights: Tensor, synth_weights: Tensor, device
):
...
def get_full_features(path, wavlm, device):
    ...
    features_list, _ = wavlm.encoder.transformer.extract_features(
...
def extract(
    ...
    for i, row in pb:
        ...
        matching_pool, synth_pool = path2pools(
            row.path, wavlm, match_weights, synth_weights, device
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compute matched wavlm features for a librispeech dataset"
    )
